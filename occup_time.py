...
def do():
    ...
    def do_per_figure_type():
        fig = plt.figure(figsize=(12, 10))
        cmap = mpl.colormaps['rainbow']
        if figure_style=='3D':
            ax =fig.add_subplot(111,projection='3d')
        elif figure_style=='heatmap':
        ...
        for index, file in enumerate(config.occup_selected_files):
            ...
            if time_this_file_fs>config.occup_t_tot:
                break
            data = np.loadtxt(file)
            if index==0:
                data_first=np.array(data)
            if Substract_initial_occupation:
            ...
            if figure_style=='3D':
                figtemp=ax.scatter(data[:,0]*Hatree_to_eV, data[:,1], zs=time_this_file_fs/1000, zdir='y',c=[time_this_file_fs/1000]*data.shape[0],cmap=cmap, vmin=0,vmax=config.occup_t_tot/1000)
            elif figure_style=='heatmap':
                figtemp=ax.scatter(data[:,0]*Hatree_to_eV, data[:,1], c=[time_this_file_fs/1000]*data.shape[0],cmap=cmap, vmin=0,vmax=config.occup_t_tot/1000)               
        ...
        if figure_style=='heatmap':
            ...
            if Substract_initial_occupation:
                ...
                ax.set_ylabel('f')
        if Substract_initial_occupation:
            fig.savefig('delta_occup_time_Ttot%dfs_Step%dfs_%s.png'%(config.occup_t_tot,config.occup_timestep_for_selected_file_ps*1000,figure_style), bbox_inches="tight")
        else:
            fig.savefig('occup_time_Ttot%dfs_Step%dfs_%s.png'%(config.occup_t_tot,config.occup_timestep_for_selected_file_ps*1000,figure_style), bbox_inches="tight")
    if output_all_figure_types:
        for figure_style in ['3D','heatmap']:
            for Substract_initial_occupation in [True,False]:
                do_per_figure_type()
    else:
        do_per_figure_type()
    config.end()